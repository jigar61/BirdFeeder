...
export class Bird {
    ...
    update(dt, W, H, feederX, feederY, allBirds) {
        ...
        if (this.type === 'cat' || this.type === 'squirrel') {
            if (this._groundOffset === undefined)
                this._groundOffset = Math.random() * 40;
        ...
        if (this.type === 'crow' && this.crowSpawnTime > 0) {
            ...
            if (timeSinceCrowSpawn > 60 && !this.flyOutStarted) {
                ...
                if (edge === 0)
                    this.flyOutTarget = { x: Math.random() * W, y: -200 };
                else if (edge === 1)
                    this.flyOutTarget = { x: Math.random() * W, y: H + 200 };
                else if (edge === 2)
                    this.flyOutTarget = { x: -200, y: Math.random() * H };
                else
                    this.flyOutTarget = { x: W + 200, y: Math.random() * H };
            }
    ...
    aiUpdate(others, dt) {
        if (this.type === 'hawk') {
            ...
            if (threat && dist(this, threat) < 300)
                this.flee(threat, dt);
            else
                this.wander(dt);
        }
