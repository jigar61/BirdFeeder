...
export class Game {
    constructor(canvasId) {
        ...
        this.lastCatSpawn = 0;
        this.lastSnakeSpawn = 0;
        this.lastRatSpawn = 0;
        this.feederImg = null;
        this.assetsLoaded = false;
        this.configuredSeeds = 100;
        this.keys = {};
        ...
        this.mouseDownX = 0;
        this.mouseDownY = 0;
        this.configuredDifficulty = 'normal';
    ...
    setupEventListeners() {
        ...
        this.canvas.addEventListener('mousemove', (e) => {
            if (!this.gameState.running || !this.selectedBird)
                return;
            const rect = this.canvas.getBoundingClientRect();
            const x = e.clientX - rect.left;
            const y = e.clientY - rect.top;
        ...
        this.canvas.addEventListener('pointerdown', (e) => {
            if (!this.gameState.running || e.pointerType === 'mouse')
                return;
        ...
        this.canvas.addEventListener('pointermove', (e) => {
            if (!this.gameState.running || !this.selectedBird || e.pointerType === 'mouse')
                return;
            const rect = this.canvas.getBoundingClientRect();
            const x = e.clientX - rect.left;
            const y = e.clientY - rect.top;
    ...
    showLevelUpNotification(levelConfig) {
        ...
        if (titleEl)
            titleEl.innerText = `⭐ Level ${levelConfig.level}!`;
        if (descEl)
            descEl.innerText = `Welcome to ${levelConfig.description}`;
    ...
    spawnInitial() {
        ...
        const chosen = speciesSelect ? speciesSelect.value : 'hawk';
    ...
    spawnBird(type, x, y, isPlayer = false) {
    ...
    updateEnvironmentalEnemies(scaledDt) {
        ...
        for (const creature of this.gameState.environmentalEnemies) {
            ...
            if (creature.x < -margin)
                creature.x = this.W + margin;
            if (creature.x > this.W + margin)
                creature.x = -margin;
            if (creature.y < -margin)
                creature.y = this.H + margin;
            if (creature.y > this.H + margin)
                creature.y = -margin;
    ...
    resetGame(pause = true) {
        ...
        if (difficultySelect) {
            this.configuredDifficulty = difficultySelect.value || 'normal';
        }
    ...
    gameLoop() {
        ...
        if (player && this.gameState.running) {
            ...
            const margin = 50;
            if (player.x < -margin)
                player.x = this.W + margin;
            if (player.x > this.W + margin)
                player.x = -margin;
            if (player.y < -margin)
                player.y = this.H + margin;
            if (player.y > this.H + margin)
                player.y = -margin;
        }
        ...
        // Collisions
        CollisionSystem.handleCollisions(this.gameState.birds, (eaten, eater) => {
            this.gameState.score++;
            this.updateScore();
    ...
    render() {
        ...
        if (this.assetsLoaded && this.feederImg) {
            const fW = (this.feederImg.width || this.feederImg.naturalWidth || feederWDefault);
            const fH = (this.feederImg.height || this.feederImg.naturalHeight || feederHDefault);
        ...
        for (const seed of this.gameState.seeds) {
            if (!seed.eaten)
                this.ctx.fillRect(seed.x - 2, seed.y - 2, 4, 4);
        }
        ...
        for (const bird of this.gameState.birds) {
            if (bird.alive) {
                ...
                Renderer.drawBird(this.ctx, bird.x, bird.y, bird._currentFrame, bird.type, displaySize);
            }
        }
        // Draw HUD
        ...
        if (this.gameState.running) {
            ...
            if (birdsEl)
                birdsEl.innerText = 'Birds: ' + this.gameState.birds.filter(b => b.alive).length;
            if (seedsEl)
                seedsEl.innerText = 'Seeds: ' + this.gameState.seeds.filter(s => !s.eaten).length;
            if (levelEl) {
                const levelConfig = getLevelConfig(this.gameState.score, this.configuredSeeds, this.configuredBirdType);
